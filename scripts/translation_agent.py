# copied from https://github.com/andrewyng/translation-agent/blob/main/src/translation_agent/utils.py
...
from threecobblers import CallableLLM
...
def run(model, source_lang, target_lang):
    global MODEL
    MODEL = CallableLLM(chatllm.LibChatLLM(PATH_BINDS), ['-m', model])
    print(f"I'm a translator from {source_lang} to {target_lang}.")

    while True:
        source_text = input(f'{source_lang} > ')

        r = one_chunk_translate_text(source_lang, target_lang, source_text, country='')

        print(f'{target_lang} Translation >')
        print(r)

if __name__ == '__main__':
    model = sys.argv[1] if len(sys.argv) >= 2 else ':qwen2'
    source_lang = sys.argv[2] if len(sys.argv) >= 3 else 'English'
    target_lang = sys.argv[3] if len(sys.argv) >= 4 else 'Chinese'
    run(model, source_lang, target_lang)