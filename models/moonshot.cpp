...
namespace chatllm::moonshot::moonlight
{
...
{
    Learnable2DInterpPosEmb::Learnable2DInterpPosEmb(InitContext *ctx, int height, int width, int dim)
        : pos_emb(ggml::new_tensor_3d(ctx, ggml::type::GGML_TYPE_F32, dim, height, width))
    {
    }

    int64_t Learnable2DInterpPosEmb::get_param_num(bool effective_only) const
    {
        return ggml::nelements(pos_emb);
    }

    ggml::tensor *Learnable2DInterpPosEmb::forward(ComputeContext *ctx, ggml::tensor *input, int grid_h, int grid_w)
    {
    ...
    void Learnable2DInterpPosEmb::load(const std::string &path, TensorLoader *loader)
    {
        loader->read_tensor(path + "weight", pos_emb);
    }

    PatchEmbedding::PatchEmbedding(InitContext *ctx, int out_dim, int patch_size, int pos_emb_height, int pos_emb_width, int in_dim)
    ...
    VisionTransformer::VisionTransformer(InitContext *ctx, const Config &config, int lm_hidden_size, Block *projector)
    ...
    DynamicBlock *VisualEmbeddingGeneration::do_create_transformer(InitContext *ctx, const Config &vis_config, const int lm_hidden_size)
    {
        return new VisionTransformer(ctx, vis_config, lm_hidden_size,
            new MultiModalProjector(ctx, vis_config, lm_hidden_size));
    }
    ...
    bool VisualEmbeddingGeneration::load_more(ggml::type dtype, int lm_hidden_size, const json::JSON &config)
    {
        ...
        vis_model.reset(do_create_transformer(&_ctx, vis_config, lm_hidden_size));
...
namespace chatllm
{
    REGISTER_MODEL_LOADER(MOONLIGHT,             moonshot::moonlight, 1);
    REGISTER_MODEL_LOADER(KIMI_VL,               kimi::vl, 1);
}