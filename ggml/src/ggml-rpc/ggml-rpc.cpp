...
static bool send_data(sockfd_t sockfd, const void * data, size_t size) {
    ...
    while (bytes_sent < size) {
        int block = (int)std::min<size_t>(size - bytes_sent, 50 * 1024 * 1024);
        ssize_t n = send(sockfd, (const char *)data + bytes_sent, block, 0);
        if (n < 0) {
            return false;
        }
        bytes_sent += n;
...
static bool recv_data(sockfd_t sockfd, void * data, size_t size) {
    ...
    while (bytes_recv < size) {
        int block = (int)std::min<size_t>(size - bytes_recv, 50 * 1024 * 1024);
        ssize_t n = recv(sockfd, (char *)data + bytes_recv, block, 0);
        if (n <= 0) {
            return false;
        }
        bytes_recv += n;
